...
def draw_line(console, buffer, screen_row, buffer_line, scroll_top)
  row = EDIT_TOP + screen_row
  line_index = scroll_top + screen_row
  if line_index < buffer.lines.length
    text = Editor.display_slice(buffer.lines[line_index], 0, Console::COLS)
...
def draw_all_lines(console, buffer, scroll_top)
  i = 0
  while i < EDIT_ROWS
    draw_line(console, buffer, i, nil, scroll_top)
    i += 1
  end
end
...
def adjust_scroll(buffer, scroll_top)
...
draw_all_lines(console, buffer, scroll_top)
draw_status(console, filepath, buffer, scroll_top, nil)

# Position cursor
screen_col = Editor.byte_to_display_col(buffer.current_line, buffer.cursor_x)
...
while running
  ...
  new_scroll = adjust_scroll(buffer, scroll_top)
  ...
  when :content
    ...
    draw_line(console, buffer, screen_row, nil, scroll_top)
  end
