...
{
  memcpy(buff, (uint8_t *)(FLASH_MMAP_ADDR + sector * FLASH_SECTOR_SIZE),
...
FLASH_disk_write(const BYTE *buff, LBA_t sector, UINT count)
{
  ...
  for (UINT i = 0; i < count; i++) {
    memcpy(sram_buf, (const uint8_t *)buff + i * FLASH_SECTOR_SIZE, FLASH_SECTOR_SIZE);
    ints = save_and_disable_interrupts();
    flash_range_program(offset + i * FLASH_SECTOR_SIZE, sram_buf, FLASH_SECTOR_SIZE);
    restore_interrupts(ints);
  }
