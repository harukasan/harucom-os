...
static void
highlight_region(syntax_data_t *sd, const uint8_t *loc_start, const uint8_t *loc_end,
                 uint8_t category)
{
...
{
  int32_t line = pm_newline_list_line(&sd->parser->newline_list, ptr, sd->parser->start_line);
...
visit_node(const pm_node_t *node, void *data)
{
  ...
  case PM_CALL_NODE: {
    ...
    if (!(call->base.flags & PM_CALL_NODE_FLAGS_VARIABLE_CALL)) {
      highlight_region(sd, call->message_loc.start, call->message_loc.end, HIGHLIGHT_METHOD);
    }
    break;
  }
  case PM_DEF_NODE: {
    const pm_def_node_t *def = (const pm_def_node_t *)node;
    highlight_region(sd, def->name_loc.start, def->name_loc.end, HIGHLIGHT_METHOD);
    break;
  }
  case PM_SYMBOL_NODE: {
  ...
  if (opens_block) {
    ...
    switch (PM_NODE_TYPE(node)) {
    case PM_CLASS_NODE:
      closing = ((pm_class_node_t *)node)->end_keyword_loc;
      break;
    case PM_MODULE_NODE:
      closing = ((pm_module_node_t *)node)->end_keyword_loc;
      break;
    case PM_SINGLETON_CLASS_NODE:
      closing = ((pm_singleton_class_node_t *)node)->end_keyword_loc;
      break;
    case PM_DEF_NODE:
      closing = ((pm_def_node_t *)node)->end_keyword_loc;
      break;
    case PM_IF_NODE:
      closing = ((pm_if_node_t *)node)->end_keyword_loc;
      break;
    case PM_UNLESS_NODE:
      closing = ((pm_unless_node_t *)node)->end_keyword_loc;
      break;
    case PM_WHILE_NODE:
      closing = ((pm_while_node_t *)node)->closing_loc;
      break;
    case PM_UNTIL_NODE:
      closing = ((pm_until_node_t *)node)->closing_loc;
      break;
    case PM_FOR_NODE:
      closing = ((pm_for_node_t *)node)->end_keyword_loc;
      break;
    case PM_CASE_NODE:
      closing = ((pm_case_node_t *)node)->end_keyword_loc;
      break;
    case PM_CASE_MATCH_NODE:
      closing = ((pm_case_match_node_t *)node)->end_keyword_loc;
      break;
    case PM_BEGIN_NODE:
      closing = ((pm_begin_node_t *)node)->end_keyword_loc;
      break;
    case PM_BLOCK_NODE:
      closing = ((pm_block_node_t *)node)->closing_loc;
      break;
    case PM_LAMBDA_NODE:
      closing = ((pm_lambda_node_t *)node)->closing_loc;
      break;
    default:
    ...
    if (is_clause) {
      ...
      size_t body_end = (node_end > node_start) ? node_end + 1 : node_start + 2;
...
mrb_ruby_syntax_analyze(mrb_state *mrb, mrb_value klass)
{
  ...
  if (source_len <= 0) {
    mrb_iv_set(mrb, result, MRB_SYM(highlight_map), mrb_str_new(mrb, "", 0));
    mrb_iv_set(mrb, result, MRB_SYM(indent_levels), mrb_str_new(mrb, "", 0));
    return result;
  }
  ...
  mrb_iv_set(mrb, result, MRB_SYM(indent_levels), mrb_str_new(mrb, (const char *)indent, lines));
...
mrb_picoruby_ruby_syntax_gem_init(mrb_state *mrb)
{
  ...
  mrb_define_module_function_id(mrb, mod, MRB_SYM(analyze), mrb_ruby_syntax_analyze,
                                MRB_ARGS_REQ(1));

  class_Result = mrb_define_class_under_id(mrb, mod, MRB_SYM(Result), mrb->object_class);
  mrb_define_method_id(mrb, class_Result, MRB_SYM(highlight_map), mrb_result_highlight_map,
                       MRB_ARGS_NONE());
  mrb_define_method_id(mrb, class_Result, MRB_SYM(indent_level), mrb_result_indent_level,
                       MRB_ARGS_REQ(1));
}
