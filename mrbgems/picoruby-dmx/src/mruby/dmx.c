...
/* DMX._init(unit_name, txd_pin) -> Integer (claimed DMA channel).
 * The keyword front end lives in mrblib/dmx.rb. */
static mrb_value
mrb_dmx__init(mrb_state *mrb, mrb_value self)
{
  ...
  return mrb_fixnum_value(channel);
}
...
mrb_picoruby_dmx_gem_init(mrb_state *mrb)
{
  ...
  mrb_define_module_function_id(mrb, mod, MRB_SYM(_init), mrb_dmx__init, MRB_ARGS_REQ(2));
