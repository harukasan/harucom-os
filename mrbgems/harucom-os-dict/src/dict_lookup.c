/*
 * Platform-independent HCDK dictionary parsing and lookup.
 *
 * The dictionary image is position independent: every offset inside it is
 * relative to the header start, so the only thing that differs between
 * platforms is where the image lives. The platform supplies that through
 * dict_region_base(); everything below reads from it.
 *
 * On RP2350 the base is the XIP-mapped flash region (ports/rp2350). A browser
 * build loads the image into a heap buffer and returns that instead.
 */
