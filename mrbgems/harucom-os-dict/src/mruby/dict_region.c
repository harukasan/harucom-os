...
mrb_im_skk_lookup(mrb_state *mrb, mrb_value klass)
{
  ...
  int count = dict_skk_lookup(reading, (int)reading_len, candidates, lengths, MAX_CANDIDATES);
...
{
  struct RClass *class_IM = mrb_define_class_id(mrb, MRB_SYM(InputMethod), mrb->object_class);

  mrb_define_class_method_id(mrb, class_IM, MRB_SYM_Q(dict_available), mrb_im_dict_available,
                             MRB_ARGS_NONE());
  mrb_define_class_method_id(mrb, class_IM, MRB_SYM(skk_lookup), mrb_im_skk_lookup,
                             MRB_ARGS_REQ(1));
  mrb_define_class_method_id(mrb, class_IM, MRB_SYM(tcode_lookup), mrb_im_tcode_lookup,
                             MRB_ARGS_REQ(2));
}
