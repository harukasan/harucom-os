...
// by the platform port that renders them (ports/rp2350/dvi_output.c).
//
...
// addressing stays valid when the active column count (dvi_text_cols) changes
// with the text scale. The active grid is dvi_text_cols x dvi_text_rows.
...
// Current applied text scale (1 = native, 2 = half-resolution 2x scaled) and
// the per-scale active grid dimensions precomputed in dvi_text_set_font. Index
// 0 is unused. The platform applies a pending scale by copying the table entry
// into dvi_text_cols/dvi_text_rows.
extern int dvi_text_scale;
extern int dvi_text_cols_for_scale[3];
extern int dvi_text_rows_for_scale[3];
