...
class BDFGlyph
  attr_accessor :encoding, :width, :height, :x_offset, :y_offset, :bitmap_rows
...
def render_glyph(glyph, fbb_width, fbb_height, fbb_y_offset)
  ...
  glyph.bitmap_rows.each_with_index do |row_bits, i|
    ...
    # Shift glyph bits to align with x_offset within the bounding box
    shift = (glyph.width - fbb_width) + (glyph.x_offset)
    if shift > 0
      row_bits <<= shift
    elsif shift < 0
      row_bits >>= -shift
...
def render_glyph_bytes(bdf, code_or_glyph, bytes_per_glyph)
  ...
  if glyph
    render_glyph(glyph, bdf.fbb_width, bdf.fbb_height, bdf.fbb_y_offset)
  else
