...
if bold_bdf
  ...
  if options[:jis]
    ...
    # Bitmap layout: [ch0_regular, ch0_bold, ch1_regular, ch1_bold, ...].
    # The second font struct (font_<wide_name>) uses glyph_stride to access
    # only the regular glyphs, skipping bold entries.
    num_chars = 94 * 94
    ...
    # Interleaved regular + bold for ASCII/Latin range.
    # Bitmap layout: [ch0_regular, ch0_bold, ch1_regular, ch1_bold, ...].
  ...
  # JIS X 0208 from Unicode-encoded BDF (e.g. otf2bdf output).
  # Converts Unicode codepoints to JIS linear index via EUC-JP mapping.
  num_chars = 94 * 94
  ...
  # JIS X 0208 from JIS-encoded BDF.
  # Linear index = (ku-1)*94 + (ten-1), ku/ten 1-indexed.
  # To convert a JIS code: ku = (code >> 8) - 0x20, ten = (code & 0xFF) - 0x20.
  num_chars = 94 * 94
  ...
  # ASCII/Latin mode. Generates glyphs for the given character range.
  range_parts = options[:range].split("-")
  first_char = Integer(range_parts[0])
  last_char = Integer(range_parts[1])
  num_chars = last_char - first_char + 1
