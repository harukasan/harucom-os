...
def generate_jis_from_unicode_header(font_name, bdf)
  guard = "DVI_FONT_#{font_name.upcase}_H"
  bytes_per_row = (bdf.fbb_width + 7) / 8
  bytes_per_glyph = bytes_per_row * bdf.fbb_height
  num_chars = 94 * 94
  ...
  # Check if widths vary (proportional wide font)
  widths = jis_glyphs.values.map { |g| g.dwidth > 0 ? g.dwidth : bdf.fbb_width }
  is_proportional = widths.uniq.length > 1

  lines = []
  lines << "// Generated by bdf2c.rb (JIS X 0208, converted from Unicode-encoded BDF)"
  ...
  if is_proportional
    ...
    num_chars.times.each_slice(16) do |indices|
      vals = indices.map { |idx|
        g = jis_glyphs[idx]
        g ? (g.dwidth > 0 ? g.dwidth : bdf.fbb_width) : bdf.fbb_width
      }
...
if bold_bdf
  ...
  header = generate_jis_from_unicode_header(font_name, bdf)
  write_output(header, options,
    "#{font_name}, #{bdf.fbb_width}x#{bdf.fbb_height}, JIS from Unicode, #{bdf.glyphs.size} glyphs")
elsif options[:jis]
