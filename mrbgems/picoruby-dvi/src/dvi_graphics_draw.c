...
static void
image_affine_bounds(int image_width, int image_height, int origin_x, int origin_y, float m00,
                    float m01, float m10, float m11, float tx, float ty, int fb_width,
                    int fb_height, int *out_min_x, int *out_min_y, int *out_max_x, int *out_max_y,
                    float *out_inv00, float *out_inv01, float *out_inv10, float *out_inv11,
...
void
dvi_graphics_draw_image_affine(uint8_t *framebuffer, int fb_width, int fb_height,
                               const uint8_t *data, int image_width, int image_height, int origin_x,
                               int origin_y, float m00, float m01, float m10, float m11, float tx,
                               float ty)
{
  int dst_min_x, dst_min_y, dst_max_x, dst_max_y;
  float inv00, inv01, inv10, inv11, etx, ety;
  image_affine_bounds(image_width, image_height, origin_x, origin_y, m00, m01, m10, m11, tx, ty,
                      fb_width, fb_height, &dst_min_x, &dst_min_y, &dst_max_x, &dst_max_y, &inv00,
                      &inv01, &inv10, &inv11, &etx, &ety);
  ...
  for (int dy = dst_min_y; dy < dst_max_y; dy++) {
    ...
    for (int dx = dst_min_x; dx < dst_max_x; dx++) {
      ...
      if (sx < 0 || sx >= image_width || sy < 0 || sy >= image_height) continue;
...
void
dvi_graphics_draw_image_masked_affine(uint8_t *framebuffer, int fb_width, int fb_height,
                                      const uint8_t *data, const uint8_t *mask, int image_width,
                                      int image_height, int origin_x, int origin_y, float m00,
                                      float m01, float m10, float m11, float tx, float ty)
{
  int dst_min_x, dst_min_y, dst_max_x, dst_max_y;
  float inv00, inv01, inv10, inv11, etx, ety;
  image_affine_bounds(image_width, image_height, origin_x, origin_y, m00, m01, m10, m11, tx, ty,
