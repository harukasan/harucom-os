...
// font cache and the 16-color palette. The platform port supplies the renderer
// and the VRAM storage (see include/dvi_text_internal.h).
//
// Cells are addressed with a fixed row stride (TEXT_VRAM_STRIDE = the maximum
...
int dvi_text_rows_for_scale[3] = {0, DVI_TEXT_MAX_ROWS, DVI_TEXT_MAX_ROWS / 2};
...
{
  dvi_text_cell_t *line = &dvi_text_write_vram[phys * TEXT_VRAM_STRIDE];
...
dvi_text_set_font(const dvi_font_t *font)
{
  ...
  for (int scale = 1; scale <= 2; scale++) {
    ...
    dvi_text_cols_for_scale[scale] = cols;
    dvi_text_rows_for_scale[scale] = rows;
  }
...
dvi_text_put_char(int col, int row, char ch, uint8_t attr)
{
  ...
  dvi_text_cell_t *c = &dvi_text_write_vram[phys * TEXT_VRAM_STRIDE + col];
  c->ch = (uint8_t)ch;
  c->attr = attr;
  c->flags = 0;
}
...
dvi_text_put_wide_char(int col, int row, uint16_t ch, uint8_t attr)
{
  ...
  dvi_text_cell_t *left = &dvi_text_write_vram[phys * TEXT_VRAM_STRIDE + col];
  dvi_text_cell_t *right = &dvi_text_write_vram[phys * TEXT_VRAM_STRIDE + col + 1];
...
dvi_text_clear(uint8_t attr)
{
  ...
  memset(dvi_text_write_row_has_wide, 0, DVI_TEXT_MAX_ROWS);
...
dvi_text_clear_range(int col, int row, int width, uint8_t attr)
{
  ...
  if (width <= 0) return;
  int phys = dvi_text_physical_row(row, dvi_text_write_scroll_offset);
  dvi_text_cell_t *line = &dvi_text_write_vram[phys * TEXT_VRAM_STRIDE + col];
...
dvi_text_get_attr(int col, int row)
{
  ...
  int phys = dvi_text_physical_row(row, dvi_text_write_scroll_offset);
  return dvi_text_write_vram[phys * TEXT_VRAM_STRIDE + col].attr;
}
...
dvi_text_set_attr(int col, int row, uint8_t attr)
{
  if (col < 0 || col >= dvi_text_cols || row < 0 || row >= dvi_text_rows) return;
  int phys = dvi_text_physical_row(row, dvi_text_write_scroll_offset);
  dvi_text_write_vram[phys * TEXT_VRAM_STRIDE + col].attr = attr;
}
...
dvi_text_read_line(int row, dvi_text_cell_t *dst)
{
  ...
  int phys = dvi_text_physical_row(row, dvi_text_write_scroll_offset);
  memcpy(dst, &dvi_text_write_vram[phys * TEXT_VRAM_STRIDE], dvi_text_cols * sizeof(dvi_text_cell_t));
}
...
dvi_text_write_line(int row, const dvi_text_cell_t *src)
{
  ...
  memcpy(&dvi_text_write_vram[phys * TEXT_VRAM_STRIDE], src, dvi_text_cols * sizeof(dvi_text_cell_t));
