...
## M2c 実機確認結果 (2026-07-06, 全項目合格)

測定器: RIGOL DS1104Z Plus、測定点は UART 一次側 (GPIO20)。灯体は M0/M1 と同じ
SHEHDS LED Spot 80W (13ch モード、アドレス 1)。確認は `rootfs/app/dmx_check.rb` の
モード 1-5 + B で実施。
...
| 項目 | 結果 |
|---|---|
| 起動ログ | `DMX: UART1 250000 baud 8N2, TX=GPIO20, DMA ch3, TIMER1 alarm 0` (期待どおり) |
| リフレッシュレート | 40.0 Hz 張り付き (512/160/26 slots すべて、30Hz 降格なし) |
| DMX.set | dimmer フェード動作 (モード 1) |
| DMX.set_range | pan スイープ + dimmer 保持、ch1-6 一括書き込みが正しくデコード (モード 2) |
| active_slots= | 512/160/26 切り替えでフレーム長のみ変化、rate 40.0 Hz 維持 (モード 3) |
| Core0 非ブロック | 512ch 送信中も画面更新・キー応答が維持 (M2 DoD) |
| フレーム衝突なし (R12) | データ帯 22.6ms + アイドル 2.3ms の繰り返しを波形で確認、末尾欠けなし |
| deadman (R19) | keepalive 停止 ~500ms で自動消灯 (readback 255→0)、キーで復灯・維持 |
| blackout | 全 ch 0 で消灯 |
...
| 測定対象 | 実測値 | 公称値 / 規格 |
|---|---|---|
| BREAK 幅 | 246.5 µs (-Width 自動計測) | 公称 176 µs + IRQ 遅延、規格 ≥88 µs |
| MAB | 14.5 µs (Period 261.0 µs − BREAK 246.5 µs) | 公称 12 µs + IRQ 遅延、規格 ≥8 µs |
| ビットタイム | 4 µs/bit (カーソル 2bit=8.000µs, 1/dx=125.0kHz) | 250000 baud 一致 |
| フレーム周期 | 25.0 ms | rate 40.0 Hz と整合 |
| 論理レベル | -40 mV / 3.32 V | 3.3V フルスイング (R3 再確認) |

補足:

- BREAK/MAB の伸び (+70 µs / +2.5 µs) は DMX アラーム IRQ (優先度 0x80) が PIO-USB SOF
  (0x00)、mruby tick (0x20)、スケジューラの割り込み禁止区間に追い越される分。伸びる方向に
  しか働かず規格内 (BREAK 上限は事実上なし)。詰める場合は TIMER1_IRQ_0 の優先度を上げる
  手があるが不要と判断。
- フレーム周期のカーソル実測 24.70 ms (40.48 Hz) は端点が「BREAK 終端 → 次 BREAK 内」の
  ため 1 BREAK 分短く見えたもの。ソフト側の 1 秒窓計測 (40 フレーム) が 40.0 Hz を示して
  おり、repeating timer は前回予定時刻基準でドリフトしない。
- 同一電源サイクル内で Q 終了 → 再実行したときの init/start 冪等性は M3 のベンチで確認済み
  (research-03)。
