...
## 現在の状態（branch: `wasm-text-mode`, `main` から24コミット先行）
...
未コミットの作業ツリー変更は無し（`rake wasm:test` 13/13 PASS = ブート+描画+入力+IME 辞書+グラフィックス+音声）。
...
- **音声** ✅完了（`1054773`）: `ports/posix/pwm_audio_wasm.c` が synth の ring buffer を
  ...
  1極 IIR で再現。`audio_demo.rb` は `board/pad`(ADC)依存で wasm 未投入のため、現状は
  IRB で `PWMAudio` 直叩きで鳴らす（ADC を足せば demo も動く）。
