...
## 現在の状態（branch: `wasm-text-mode`, `main` から12コミット先行）

```
88cb1ad Wire browser keyboard input into the OS
fccdf69 Boot the real system.rb userland in the browser
59a0aa1 Anchor the smoke-test palette constants to their table indices
946f40b Gate the wasm canvas blit on the commit counter and drive it from rAF
f805f1d Compile the uni2jis table into libmruby on every platform
58c593f Share the DVI put_string body between normal and bold
b078262 Make DVI.wait_vsync yield to the browser and verify the wasm boot
1f9a2fd Render the DVI text mode to a browser canvas
2a9e5e7 Extract the DVI text core into a shared, platform-independent module
b605c1e Clean up the wasm boot entry
c8db7e6 Boot Harucom OS in the browser via picoruby-wasm (Phase 1)
f3e09aa Add wasm build config and HAL gem for browser Harucom OS
```

未コミットの作業ツリー変更は無し（`rake wasm:test` 7/7 PASS、ブート+描画+入力）。
...
- **Phase 4 グラフィックス**: `dvi_get_framebuffer` を blit、`dvi_set_mode` 切替。
  レビュー #3（`active_mode` 未読 / `scale` スタブ、`dvi_wasm.c:99,120`）をここで正す。
- **音声**: `ports/posix/pwm_audio_wasm.c` → WebAudio。
- **IME/辞書**: `InputMethod.dict_available?/skk_lookup/tcode_lookup` は `harucom-os-dict`
  の C 実装。wasm build に未投入のため、SKK/T-Code 変換は現状 NoMethodError になる
  （ブート + ASCII 入力には不要なので Phase 2b では未対応）。辞書バイナリ + C を
  emscripten に取り込む。
...
| 3 | graphics スタブ（`active_mode` 未読、`scale=2` で 320/240 返すが blit は 640×480 固定） | `dvi_wasm.c:99,120` | Phase 4 |
