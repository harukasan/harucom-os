...
- **(a) 高音域のクリック（プツプツ）= audio ISR の timing ジッタ**。性質: 音程依存
  ...
  間隔**）。**最有力原因: PIO-USB の HID ポーリングが Core0 で audio の alarm_pool ISR を
  押しのける**（`usb_host.c:60` SOF も alarm_pool、`pwm_audio_port.c` audio も alarm_pool、
  優先度0でも PIO-USB に負ける）。**本修正案: 出力段を DMA+DREQ ハード駆動に作り替え**
  （`ports/rp2350/pwm_audio_port.c` のみ。synth/mruby API/wasm 無変更）。サンプルクロックが
  CPU/割り込みから独立し、干渉源が何であれジッタ=クリックが消える。要実機検証。
- **(b) ピッチ誤差 +0.78%**（A4 が ~443Hz）。原因: audio タイマ `-(1000000/22050)` の整数
  切り捨て（45.35us→45us → 実 22222Hz）。**44100 は不可**（22.68us→22us=45454Hz, +3% 悪化）。
  **40000 か 50000 なら整数 us でちょうど**（誤差0、dt も半減）。**※ 40000 化を一度実装→
  クリック((a))がむしろ悪化したため撤回**（ジッタが本質で、レート上げは締切を厳しくするだけ
  だった）。DMA 化((a)) すれば任意レートで正確になるので、ピッチは DMA 化に合わせて是正するのが
  筋。単独でやるなら 40000/50000 か、DMA の fractional 分周。
