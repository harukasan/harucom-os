...
音声解析ツールは `scripts/` に集約（`measure_audio.cjs` 合成計測、`analyze_recording.py` 実機録音解析）。
`proto_antialias.cjs` は手法選定の役目完了で削除済み。実機録音 `tmp/*.wav` は `.gitignore` で
ローカル保持。作業ツリーはクリーン。`rake wasm:test` **18/18 PASS** =
...
- **Phase 2b**（`fccdf69`, `88cb1ad`）= **実 `system.rb` 起動 + キーボード入力**:
  ...
  - B-5 `wasm/index.html` で DOM key → HID usage 変換 → `_harucom_kbd_set_state`
    （後に `js/keyboard.js` へ分割）。`_harucom_kbd_set_state` を emcc exports に追加。
  - `wasm/run_node.cjs` を実ブート検証に書換え（IRB バナー到達 + framebuffer 描画 +
    `9-7`↵→`=> 2` の E2E キーストローク）。後に node:test 化し `wasm/tests/*.test.cjs` へ分割。
...
- **キーボード**: `usb_host_wasm.c` が JS 注入の HID レポート（modifier + keycodes[6]）を
  ...
  は全て無変更で再利用。`js/keyboard.js` が `keydown`/`keyup`→HID usage→`harucom_kbd_set_state`。
...
- **JS run loop**: `wasm/js/runloop.js`（`main.js` が配線）が `mrb_tick_wasm`→`mrb_run_step`→
  （frame_count が進んだ時のみ）`blit`→`requestAnimationFrame`。framebuffer(RGB332)→canvas は LUT 変換。
...
コードなので**実機でも同一**。`scripts/measure_audio.cjs`(スペクトル) と proto_antialias(手法比較、
選定後に削除) で確定し、独立検証エージェント3つが HIGH 信頼度で一致。
...
1. **square/saw に polyBLEP ✅コミット済み（`3c52551`）+ 実機確認済み**:
   ...
   `scripts/analyze_recording.py` で解析し、**実機で可聴域エイリアスが抑制され倍音はクリーン**
   （G4 録音で確認）= polyBLEP は実機で機能。共有 synth なので実機の音色も滑らかになる。
   `rake wasm:test` 18/18 PASS。

### 次の最優先（音声の残課題＝後回しにした2件、実機専用）

実機録音（`tmp/harucom_square_*.wav`）を `scripts/analyze_recording.py`（無音帯バースト＋
...
3. **音声解析ツール（`scripts/` に集約）**: `harucom_audio_measure_*`(C) + `scripts/measure_audio.cjs`
   は synth 回帰計測、`scripts/analyze_recording.py` は実機録音解析（上記(a)(b)の確定に使用）。使い方は
   [doc/wasm-audio-measurement.md](../wasm-audio-measurement.md)。`proto_antialias.cjs` は手法選定の
   役目完了で削除済み（根拠は本節と下記「音声」に記録）。
4. **診断ログの削除**: `js/audio.js` の `setInterval(... "audio diag: ..." ...)`
   （`6eb0448` で追加、index.html 分割で js/audio.js へ移動）はデバッグ用。音声確定後に削除する。
...
### 次の作業B: コードレビュー指摘（DVI 描画エッジケース）— 全件決着

下表参照。項目2/6 は `dvi_wasm.c` で対応済み（`138bc10`）。項目4/5 は「実機と同一挙動・防御的」
として据え置き（ユーザー判断、実機準拠を優先し防御ガードで乖離させない）。**残る任意作業は
項目2 の writer 側（orphan セルの根本除去）のみ**で、共有 `dvi_text.c` を触るため要実機検証
（`rake distclean && rake` + 実機 FIFO underflow 確認をユーザーに依頼）。優先度低。
...
- **音声** ✅ほぼ完了（合成〜再生は安定。square/saw のエイリアシングも polyBLEP で解消・
  ...
  最終アーキテクチャを記す:
  - **再生経路 = AudioWorklet**（`js/audio.js` + `js/audio-worklet.js`、`ad08a2d`/`6eb0448`）。`process()` が
  ...
  - **フロー制御 = 時間ベース**（`6eb0448`）。pump は「前回 pump からの経過実時間 × rate」
    ...
    になった）。`TARGET=3072`(~140ms)。`js/audio.js` に毎秒の診断ログ
  ...
  - **検証**: ヘッドレス `wasm/tests/audio.test.cjs` が 矩形波の発振・RC 平滑・
  ...
  - **エイリアシング対策 = polyBLEP（square/saw、コミット済み `3c52551`・実機確認済み）**: ユーザー
    ...
    手法選定の根拠: `proto_antialias.cjs`(選定後に削除) で naive / polyBLEP-2点 / oversample 2x-4x-8x /
...
- **ADC パッド** ✅完了（`feeaed8`）: `harucom_wasm.c` に wasm 専用 `ADC` クラス
  (`read_raw` が JS 注入値を返す)+ `harucom_pad_set(index, raw)`。`js/pads.js` の画面下
...
| # | 内容 | 箇所 | 対応時期 |
|---|---|---|---|
| ~~2~~ | ~~stray WIDE_R 描画差異（全角左半を半角上書きで右半残り、以降の行が水平シフト）~~ → wasm レンダラを実機に忠実化（WIDE_R を WIDE_L と同じ 12px+2セル消費に）。**残: orphan を根本から無くす writer 側（上書き時に相方クリア）は共有 `dvi_text.c`=要実機検証で別途** | `dvi_wasm.c` | ✅完了（レンダラ） |
| ~~4~~ | ~~WIDE_L 最終列で 1byte OOB read/write（防御的、wasm は buffer タイト）~~ → **据え置き（ユーザー判断）**。wasm レンダラの `grow[col+1]` は実機の `glyph_ptr` と同一挙動で、防御ガードを入れると逆に実機から乖離する。正規経路は `put_wide_char` が防御済み、実害は軽微。実機一致を優先 | `dvi_text.c`/`dvi_wasm.c` | 据え置き（実機一致） |
| ~~5~~ | ~~`utf8_decode` が NUL 跨ぎ read（途中で切れたマルチバイトで 1-3byte 余分）防御的~~ → **据え置き（ユーザー判断）**。共有コードで board と完全同一挙動。防御的（正規入力では起きない）で実機と一致しているため変更しない | `dvi_text.c` | 据え置き（実機一致） |
| ~~6~~ | ~~右マージン/画面外を `palette[0]` で塗る（実機は固定黒0、背景色変更時のみ相違）~~ → 固定黒 `0x00` に変更 | `dvi_wasm.c` | ✅完了 |
...
- `bundle exec rake wasm:build` — libmruby を emcc ビルド + リンク。**gem 追加・新規
  ...
  `CLEAN=1 rake wasm:build`**（presym/host を再構築）。`index.html` / `js/` のみの変更は
  `rake wasm:server` の `stage_index!` で反映（リビルド不要）。rootfs C 配列 `ruby_scripts.h` は
  `build/ruby_scripts.h` に生成（実機 CMake と同じ場所に統一。gem src ツリーには置かない）。
  `harucom-os-wasm/mrbgem.rake` が `build/` を include パスに追加。
- `bundle exec rake wasm:test` — jsdom ヘッドレス smoke（`wasm/tests/*.test.cjs`、9ファイル/**20 gate**、
  node:test、ファイル毎に VM ブート）。
...
- `scripts/measure_audio.cjs` — synth スペクトル計測（FFT/Blackman-Harris/倍音・非倍音分離）
- `scripts/analyze_recording.py` — 実機録音(WAV)のスペクトル＋クリック(無音帯バースト/位相跳び/
  underrun ドロップアウト)解析（numpy+ffmpeg。音声残課題(a)(b)の確定に使用）
...
- `wasm/index.html` — マークアップ + `<script>`（harucom.js classic + `js/main.js` module）
- `wasm/js/{main,display,keyboard,audio,audio-worklet,pads,runloop}.js` — ブラウザ JS（VM 配線 /
  canvas blit / キーボード / 音声 worklet / パッド / rAF run loop）
- `wasm/tests/{harness,boot,render,input,preempt,ime,graphics,audio,pad}.cjs` — node:test ヘッドレス
  smoke（`harness.cjs` の `boot()` を各 `*.test.cjs` が呼ぶ。ブート+描画+入力+...+パッドを個別検証）
