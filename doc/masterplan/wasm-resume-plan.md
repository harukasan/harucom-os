...
## 現在の状態（branch: `wasm-text-mode`, `main` から36コミット先行）
...
未コミットの作業ツリー変更は無し（`rake wasm:test` **18/18 PASS** = ブート+描画+入力+
プリエンプション+IME 辞書+グラフィックス+音声(合成/フィルタ/underrun ガード/note-off
declick)+ADC パッド）。
...
1. **sine の残ノイズ（~6-7% 非倍音、-23dB）の切り分け**: 「音声」節の残課題参照。
   スペクトル測定（広帯域 vs 非倍音スプリアス vs 計測漏れ）で synth 量子化（実機同等=
   仕様）か wasm 固有かを確定。実機同等ならユーザーに「これは PWM シンセの素特性」と
   報告して完了でよい。改善するなら sin LUT 線形補間（共有 synth、要承認）を検討。
   ヘッドレス計測スクラッチの雛形は git 履歴のこのセッション（`harucom_audio_pull` を
   update+pull で連続取得 → 連続周波数 DFT で fundamental/harmonics/noise を分離）。
   注意: `pull(N)` は **N < リング在庫(1023)** にしないと underrun で計測が汚れる。
2. **診断ログの削除**: `index.html` の `setInterval(... "audio diag: ..." ...)`
   （`6eb0448` で追加）はデバッグ用。音声確定後に削除する。
3. ブランチ統合: `wasm-text-mode` → `main` の取り込み（ユーザー判断）。
...
- **音声** ✅ほぼ完了（合成〜再生は安定。残: 後述の「倍音ノイズ」調査）。経緯が長いので
  最終アーキテクチャを記す:
  ...
  - **残課題（次セッション最優先）**: sine で **~6-7% の非倍音ノイズ(-23dB)** が残る
    （倍音 2-7次は -52〜-85dB で THD 0.2-0.3% と極小なので「倍音歪み」ではなく非倍音/
    広帯域成分）。ratio=1・フィルタ線形なので **synth の量子化（9bit PWM duty + 256点
    sin LUT 無補間）の素特性=実機同等**が最有力。次の手はスペクトル測定で広帯域 vs
    非倍音スプリアスを切り分け、実機相当(=仕様)か wasm 固有かを確定する。改善するなら
    **synth 側に sin LUT 線形補間**（共有コード、実機も改善、要ユーザー承認）か、
    wasm 専用に **ring を 9bit duty でなく 12bit mix で渡す高解像度経路**（実機と乖離）。
    ※ なお wasm RC LPF の実効カットオフは alpha 近似で ~2.3kHz とやや低め（実機 3.3kHz
    より僅かにこもる）。倍音を「足す」方向ではないので本ノイズの原因ではないが、忠実度を
    詰めるなら係数見直しの余地あり。
...
- `mrbgems/picoruby-pwm-audio/src/pwm_audio.{c,h}` — 共有 synth。**attack/release エンベロープ**
  （`env`/`env_target`/`ENV_MAX`/`ENV_STEP`、note on/off declick。実機にも影響）
- `mrbgems/picoruby-pwm-audio/ports/posix/pwm_audio_wasm.c` — `harucom_audio_pull`、
  RC LPF + **DC ブロッキング HPF**、**underrun でフィルタ状態を保持**
...
- **共有 synth（`pwm_audio.c`）のエンベロープが実機の音にも影響**: note on/off が ~2.9ms
  フェードイン/アウトになる（クリック除去目的、ユーザー承認済み）。実機での音の確認は
  ユーザーに依頼。`MRB_USE_DEBUG_HOOK` を実機ビルドにも入れるかは別途判断（プリエンプ
  ション hook は wasm 専用、実機はタイマ割込なので不要）。
