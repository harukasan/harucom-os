...
| 資源 | 用途 |
|---|---|
| TIMER0 alarm 0 | mruby task tick (1ms) ※既存 |
| TIMER0 alarm 2 / 3 | PIO-USB SOF / SDK 既定 alarm pool ※既存 |
| TIMER1 alarm 0 + 専用 pool | DMX フレーム state machine (BREAK→MAB→DMA) |
| TIMER1 alarm 1 + 専用 pool | pwm-audio レンダポンプ (10ms、C 自律充填) |
| DMA ch0/ch1 + `DMA_IRQ_1` | DVI (Core1) ※既存・専有 |
| DMA ch (claim) | オーディオ (リング → PWM slice4 CC、ペーサ slice8 wrap DREQ = 50kHz、ENDLESS) |
| DMA ch (claim) | DMX (ユニバース → UART1 TX FIFO, `DREQ_UART1_TX`) |

実装は新規系が DMA 完了 IRQ を使わない構成に落ち着いた (音声 = wrap DREQ + ENDLESS リング、
DMX = alarm 駆動のフレーム反復)。DVI の `DMA_IRQ_1` は不可侵。音声のサンプルレートは
clk_sys 250MHz の整数分周 50kHz (詳細は research 02 の実装結果)。
...
```
 ...
 keyboard task         │   - 編集: Editor::Buffer + RubySyntax   │
   ...
   音DMA (ペーサ wrap DREQ 50kHz) ──► sample_clock(音予約用)─┘ │
...
DMA 化・sample_clock・サンプル精度予約 (`tone_at`/`stop_at`) は実装済みで main に
マージされた。現状の正は [doc/pwm-audio.md](../pwm-audio.md)、経緯と設計判断は
[research-johakyu-02-audio-engine.md](research-johakyu-02-audio-engine.md) の実装結果を
参照。残タスクはサンプル再生 (WAV/QOA、`play_pcm` 系) のみ。
