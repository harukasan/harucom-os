...
- 検証: `fixture_check.rb` の Sweep モード (raw ch を 1 つずつ駆動、マニュアルの期待挙動を
  画面表示) と Wheel モード (color/gobo/prism の名前テーブル照合) で実機と突き合わせる。
...
`run app/fixture_check.rb` で以下を実機確認する:

1. Resolve モード: 全アサーションが PASS (readback 判定、目視不要)。
2. Sweep モード: 13ch を順に振り、マニュアルの期待挙動と一致するか目視確認 (R6)。
   CH12/13 は自動ランプなし (auto プログラム/リセット誤爆防止)、手動キーのみ。
3. Wheel モード: color/gobo/prism の名前と実際のホイール位置の一致 (R6)。
4. Identify モード: s2 の本体アドレスを 014 に設定した上で、名前どおりの灯体だけが
   点くこと (アドレス整合手順のステップ 3)。
5. Spread モード: 2 台が同報点灯し、pan がオフセット付きでチェイスすること。
6. R13: 終端有効化の上で最遠灯体 (s2) の値化けがないこと。
7. research-01 の残項目: 同一電源サイクル内で dmx_check / fixture_check を Q 終了 →
   再実行して init/start が冪等なこと (確認できたら research-01 の記述を消す)。
