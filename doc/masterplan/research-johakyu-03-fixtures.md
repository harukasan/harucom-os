...
Sweep モード中に散発したハードフォルトは、デバッガでフォルトフレームを取得して根本原因を
特定した (2026-07-07)。原因は fixture_check ではなく mruby-task のプリエンプションが再入
`mrb_vm_exec` から早期 return して callinfo を壊すバグ。分析・修正パッチ・再現手順は
[research-mruby-preempt-nested-exec.md](research-mruby-preempt-nested-exec.md) に記録し、
**別タスクとして対応する (パッチは未適用)**。
