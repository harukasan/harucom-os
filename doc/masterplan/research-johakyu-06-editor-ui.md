...
- 実機フィードバック 1 (2026-07-08): カーソル移動中に音が不安定。原因は毎キーの文字列
  churn — display_slice は 1 文字ごとに String を確保し、ステータス行 + コマンドバーの
  再描画で 1 キー約 200 アロケーション × キーリピート 20Hz が GC を蹴り、audio の
  ring buffer fill (46ms 猶予) を止めていた。対策: コマンドバー文字列のキャッシュ化、
  ステータス行の slice 回避、fix/editor-scroll-stutter ブランチをマージし同じ技法
  (ウィンドウ構文解析・スクロールと dirty の分離・DVI リングスクロール・横ジャンプ
  スクロール) を JohakyuApp に移植。リングスクロールは全画面を動かすため、同一フレーム
  で universe view とコマンドバーを再描画する。
- 実機確認 (2026-07-08、M9 DoD 達成): 分割表示・編集中ショー継続・F5/Ctrl-S 境界
  差し替え・エラー時走行継続・タイムアウト復帰を確認済み。カーソル移動中の音乱れは
  修正済み (下記フィードバック 1)。
- 実機フィードバック 2 (2026-07-08): eval タイムアウトの `Sandbox#stop` が nested-exec
  系 hardfault を再現 (`loop {}` で発生、research-mruby-preempt-nested-exec.md 参照)。
  stop をやめ suspend + サンドボックス放棄・新規作成に変更して解決。走行中タスクへの
  stop は禁止。
- 残メモ (M9 クローズ時点):
  - eval 中の音の乱れ (連続 eval 時): コンパイル + スクリプト実行のアロケーションで GC が
    audio fill (46ms 猶予) を止める。根本対策は M5 の WAV+DMA 化 (バッファ拡大単独は
    レイテンシ校正を無効化するためやらない)。
  - R9 のヒープ安定は間接確認のみ (memory_statistics はファーム構成外)。
  - fixture 行は数値 readback の簡略版 (ミニバー/実色表示は M10 の演出磨き候補)。
  - IME 辞書はデバッガ書き込みだと入らない (full UF2 を一度焼く。イメージの問題)。
  - Ctrl-C は irb 側 stop で shutdown が走らず鳴走中トーンが残り得る (灯体は deadman で
    消灯)。終了は Ctrl-Q が正。改善するなら irb run_app の ensure に音停止を足す (別件)。

## 保留: 構文エラーの詳細表示

実行時エラーはステータス行に表示済み (message + class、backtrace があれば先頭の位置も)。
構文エラーは現状 "Syntax error" の固定表示のみ: `Sandbox#compile` は true/false しか
返さず、パーサ診断 (mruby-compiler2 の `cc->diagnostic_list`、message/line/column 持ち)
は Ruby に露出していない。表示するには picoruby-sandbox の拡張 (compile 失敗時に
free_ccontext 前へ診断を退避し `Sandbox#compile_error` として返す、新 MRB_SYM のため
distclean 必要) が要るが、サブモジュールへの手入れは一旦見送り (2026-07-08 ユーザー判断)。
picoruby 本体への upstream 提案候補。
