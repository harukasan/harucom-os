...
### 実機確認結果 (2026-07-07, M4 DoD 達成)

`run app/johakyu_demo.rb` の 2 台リグで、キック同期の dimmer、プリセットの境界量子化
スワップ、テンポ変更、R15 計測 (上記) を確認した。ベンチで見つかり修正した項目:

- 音抜け: ゲートオフが目標時刻基準だったため遅延発火時にノートが即殺されていた。
  実発火時刻基準 + 再トリガ時の古いゲート破棄で解消 (688df61)。
- つまり: 初版スケジューラの query コスト。チャンク staging 再設計で解消 (3519e8d、
  実測は上記 R15)。
- 音光レイテンシ: PWM リングバッファ (1024 サンプル ≈ 46ms) と DMX フレーム (平均
  ~12ms) の経路差で光が先行。per-track latency オフセットで音を早出しし、実耳
  キャリブレーションで既定 20ms に確定 (灯体 dimmer 自体の遅れがあるため理論値 34ms
  より小さい)。会場では [/] キーで再調整する (2813c5d, 2700f42)。
- 知覚の罠: 65Hz のキックは小型スピーカーで再生されず、スネアが「拍」に聞こえて
  表拍の光が裏に見えた。キックを 110Hz へ変更し、画面に 8 ステップカーソルを追加
  (d03d12a, 119a0c3)。
- ベンチ中に mruby-task プリエンプションのハードフォルトが再発 (計 3 回目)。上流
  修正のバックポートで解消 ([research-mruby-preempt-nested-exec.md](research-mruby-preempt-nested-exec.md))。
