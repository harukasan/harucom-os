...
MRuby::CrossBuild.new("harucom-wasm") do |conf|
  ...
  # Select the mruby VM (PICORB_VM_MRUBY + the task scheduler). This also adds
  # the picoruby-mruby gem, so it does not need to be listed separately.
  # alloc_estalloc is the board's fixed-region allocator over PSRAM. The browser
  # has no such region and mrb_open fails if it is selected, so use the libc
  # allocator over the emscripten heap (as upstream build_config/picoruby-wasm.rb
  # does).
  ...
  # avoids minimum for the same reason.
