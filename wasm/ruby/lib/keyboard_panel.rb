...
class KeyboardPanel < Harucom::UI::Panel
  ...
  def key_button(label, usage, width)
    button(class: s.key | width,
           onpointerdown: -> { key_press(usage) },
           onpointerup: -> { key_release(usage) },
           onpointerleave: -> { key_release(usage) }) do
      label
    end
  end

  def toggle_button(label, which, width)
    button(class: s.toggle(state[which]) | width, onclick: -> { toggle(which) }) do
      label
    end
  end
  ...
  def render
    div(id: "keyboard", class: s.wrap) do
      i = 0
      while i < ROWS.length
        row = ROWS[i]
        div(class: s.row) do
