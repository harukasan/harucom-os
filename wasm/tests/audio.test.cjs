...
// harucom_audio_pull (the same path the browser AudioWorklet pump uses). Audio
...
describe("audio synth + pull port", () => {
  ...
  before(async () => {
    ...
    typeString("PWMAudio.init(24,25);PWMAudio.tone(0,440,PWMAudio::SQUARE,15)");
    hidType(ENTER);
    ...
    for (let k = 0; k < 3; k++) {
      drive(2000);
      const g = Module._harucom_audio_pull(lPtr, rPtr, CAP); // over-pull across refills
