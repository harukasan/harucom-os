...
// process, and a fresh boot per file is cheap). It lives outside the test glob
...
// DOM (window/document), and jsdom provides a real one, so this exercises the
// same gem init the browser does. The real target is the browser (rake
// wasm:server). This harness is for headless smoke testing.
