// Scheduler preemption gate: the opcode-budget hook must split a long busy Ruby
// loop across many mrb_run_step calls. The driver only ticks the clock between
// steps, so without the hook the whole loop would run inside a single
