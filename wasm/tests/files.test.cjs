...
// it, and can read one back out byte for byte. These run against a real VM, so
// they answer whether the OS actually sees what was written. The panel that
// draws the list is a component, covered by rake wasm:ui_test.
